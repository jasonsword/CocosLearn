...
void StarMatrix::consumeSelectedStar(Star* original)
{
	...
	selectedCount = 1;
	//LOG_DEBUG("first %d --- %d", star->getX(), star->getY());
...
void StarMatrix::updateCheck(float delta /*= 0*/)
{
	if (checkEnded())
	{
		LOG_DEBUG("no more star can be cleared!");
...
void StarMatrix::playComboEffect()
{
	...
	{
		LOG_DEBUG("combo resource not found!");
		return;
	}
