...
void XmlData::parseStarInfo()
{
	...
	if (errorno != 0)
	{
		LOG_DEBUG("xml doc parse error errorno:%d", errorno);
		return;
	}
	...
	if (root == 0)
	{
		LOG_DEBUG("xml doc parse error root not found!");
		return;
	}
	...
	if (starRoot == 0)
	{
		LOG_DEBUG("xml doc parse error starRoot not found!");
		return;
	}
...
void XmlData::saveStarInfo()
{
	...
	std::string strPath = FileUtils::getInstance()->getWritablePath() + "data.xml";
	//LOG_DEBUG("SAVE PATH: %s", strPath.c_str());
